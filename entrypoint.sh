...
echo "Creating new indexer on instance '${INPUT_AZURESEARCHINSTANCE}':"
echo $DEFINITION | jq

set +e
RESPONSE=$(curl --fail-with-body -sSD/dev/stderr \
  --location \
  --request POST "https://${INPUT_AZURESEARCHINSTANCE}.search.windows.net/indexers?api-version=${API_VERSION}" \
